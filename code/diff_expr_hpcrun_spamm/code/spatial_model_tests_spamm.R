...
# non_spatial_de_hpc
#
non_spatial_de_hpc = function(expr_data=NULL, annot=NULL){
  # Recode clusters
  meta_tmp = annot
  expr_tmp = expr_data %>%
...
#genename = 'ABCA1'
cluster = as.character(cmdargs[2])
#cluster = 'none'
stlist_fp = as.character(cmdargs[3])
#stlist_fp = '../data/visium_stlist_w_clusters_raviglioblastoma.RDS'
samplename = as.character(cmdargs[4])
#samplename = 'UKF243'
...
non_sp_models = non_spatial_de_hpc(expr_data=expr_df, 
                               annot=meta_dict[['coded_annot']][ meta_dict[['orig_annot']] == cluster ]) # NEED TO MAKE FUNCTION TO SAVE EXEC TIME

# Calculate log-fold changes
meta_tmp = unique(non_sp_models[['nonspmod']][['data']][['meta']]) %>% grep('other', ., value=T, invert=T)
avgexpr_ref = mean(non_sp_models[['nonspmod']][['data']] %>% dplyr::filter(meta != 'other') %>% dplyr::select(exprval) %>% unlist())
avgexpr_other = mean(non_sp_models[['nonspmod']][['data']] %>% dplyr::filter(meta == 'other') %>% dplyr::select(exprval) %>% unlist())

# Add results and logFC to list of data frames
non_sp_de = tibble::tibble(gene=genename,
                           cluster=meta_dict[['orig_annot']][ meta_dict[['coded_annot']] == meta_tmp ],
...
rm(meta_tmp, avgexpr_ref, avgexpr_other) # Clean environment
...
if(any(class(sp_models[['spmod']][['exp']]) == 'HLfit')){
  ...
  meta_exp_tmp = unique(sp_models[['spmod']][['exp']][['data']][['meta']]) %>% grep('other', ., value=T, invert=T)
  meta_exp_tmp = meta_dict[['orig_annot']][ meta_dict[['coded_annot']] == meta_exp_tmp ]
...
outfp = gsub('.RDS', paste0('_', samplename, '_', genename, '_', cluster, '.csv'), outfp)
